...
interface TutorPanelProps {
    ...
    nudge?: any; // The nudge object from backend response
    onClose?: () => void;
}

const TutorPanel: React.FC<TutorPanelProps> = ({ isVisible, currentTaskContext, nudge, onClose }) => {
    const [messages, setMessages] = useState<TutorMessage[]>([
        { id: 'welcome', sender: 'tutor', text: "Hallo! Ich bin dein Tutor. Frag mich, wenn du Hilfe brauchst!" }
    ]);
    const [input, setInput] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    const [responseLanguage, setResponseLanguage] = useState<'de' | 'en'>('de'); // 'de' = Deutsch, 'en' = English
    ...
    const sendMessage = async (text: string) => {
        ...
        try {
            const response = await fetch('/experiment/tutor/ask', {
                method: 'POST',
                ...
                body: JSON.stringify({
                    ...
                    task_context: {
                        ...
                        is_correct: currentTaskContext.isCorrect, // can be null/undefined
            ...
            setMessages(prev => [...prev, { id: `err-${Date.now()}`, sender: 'tutor', text: "Entschuldigung, ich habe Verbindungsprobleme." }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col h-[600px] w-full lg:w-[350px] bg-white rounded-[2rem] shadow-xl border-4 border-slate-100 overflow-hidden shrink-0 transition-all">
            {/* Header */}
            <div className="bg-purple-600 p-4 text-white flex justify-between items-center shadow-md z-10">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-lg">Dein Tutor</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={() => setResponseLanguage(prev => prev === 'de' ? 'en' : 'de')}
                        className={`relative w-12 h-6 rounded-full transition-colors duration-300 ${responseLanguage === 'de' ? 'bg-green-400' : 'bg-gray-300'}`}
                        aria-label="Toggle language"
                    >
                        <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${responseLanguage === 'de' ? 'translate-x-6' : 'translate-x-0'}`} />
                    </button>
            ...
            <div className="p-4 bg-white border-t border-slate-100">
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        className="flex-1 bg-slate-50 border-slate-200 border rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-purple-400 focus:bg-white transition-all"
                        disabled={isLoading}
                    />
                    <button
                        onClick={() => sendMessage(input)}
                        disabled={isLoading || !input.trim()}
                        className="p-2 bg-purple-600 text-white rounded-xl hover:bg-purple-700 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100"
                    >
